...
export default async function handler(req: any, res: any) {
  ...
  if (segments[0] === 'analyses' || urlPath.includes('/analyses')) {
    ...
    // POST /api/analyses (upload) - signal client to parse client-side with full pdfjs accuracy
    if (!analysisId && req.method === 'POST') {
      res.setHeader('Content-Type', 'application/json');
      return res.status(200).json({
        client_fallback: true,
        message: 'Parse uploaded PDF client-side for zero-loss page fidelity.',
      });
    }
    ...
    if (!analysis) {
      if (analysisId === 'case-1' || analysisId.includes('case-1') || analysisId.includes('moratorium')) analysis = DEMO_CASES['demo-case-1-strong-moratorium'];
      else if (analysisId === 'case-2' || analysisId.includes('case-2-weak') || analysisId.includes('weak')) analysis = DEMO_CASES['demo-case-2-weak-valid-rejection'];
      else if (analysisId === 'case-3' || analysisId.includes('no-clause') || analysisId.includes('case-2')) analysis = DEMO_CASES['demo-case-2-no-clause'];
      else if (analysisId === 'case-4' || analysisId.includes('mismatch') || analysisId.includes('case-3')) analysis = DEMO_CASES['demo-case-3-clause-mismatch'];
    }
